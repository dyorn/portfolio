...
class MissJulia3D extends React.Component {
    componentDidMount() { 
        ...
        // Initial Plane setup
        this.planeGeometry = new THREE.PlaneGeometry(1, 1, this.resolution-1, this.resolution-1)
        ...
        const xGeometry = new THREE.SphereGeometry(.35, 16, 16)
        ...
        const yGeometry = new THREE.SphereGeometry(.35, 16, 16)
        ...
        const xyGeometry = new THREE.SphereGeometry(.3, 16, 16)
        ...
        const otherGeometry = new THREE.SphereGeometry(.2, 16, 16)
    ...
    calculateJulia(z_r, z_i, noise_real, noise_im){
        ...
        for(iteration = 0; iteration < this.MAX_ITERATIONS; iteration++){
            if(this.type == 0){
                ...
                dz = this.complex_mult(dz, {real: 4*z3.real - 2*z.real, im: 4*z3.im - 2*z.im})
                if (z.real * z.real + z.im * z.im > R_squared){
                    break
                }
        ...
        // return distance
        return -Math.sqrt((z.real*z.real + z.im*z.im) / (dz.real*dz.real + dz.im*dz.im)) * Math.log(z.real*z.real + z.im*z.im)*0.5
    }
    ...
    tick() {
        ...
        let noise_real = Math.cos(elapsedTime*this.noiseTimeScale)*this.noiseScale
        ...
        for(let x = 0; x < this.resolution; x++){
            ...
            for(let y = 0; y < this.resolution; y++){
                ...
                this.planePositionAttribute.setZ(x*this.resolution + y, THREE.MathUtils.clamp(distance, -5., .3))
                z_im += z_delta
            }
            z_real += z_delta
        }
    ...
    randomizeVariables() {
        this.noiseScale = Math.random()*0.19 + .01
        this.noiseTimeScale = Math.random()*1.5 + .5
                    ...
                    <div className="flex w-full flex-wrap max-w-sm lg:w-1/2 mb-4 lg:mx-6 lg:justify-start">
                        ...
                        <div className="flex w-full boxshadow-3d-right mt-4 lg:mt-8 mb-4">
                            <p className="w-full text-sm md:text-md lg:text-lg font-extralight font-manrope m-4">
                                A 3D Julia Sets Visualization
                            </p>
                        </div>
                        <div className="flex w-full boxshadow-3d-right mt-4 lg:mt-8">
                            <p className="w-full text-sm md:text-md lg:text-lg font-extralight font-manrope m-4">
                                Please <b>click</b> on the visualization to randomize the C values.
                            </p>
                        </div>
                    </div>
                </div>
